...
@endphp
    ...
    <div class="row">
        <input type="date" id="fecha-m" class="form-control" value="{{date('Y-m-d')}}">
    </div>
    <div id="horarios-m" class="row horarios-m" >
        <div class="col-sm-6">  
            ...
            @for($i=strtotime($horario->hora_entrada_M);$i<=strtotime($horario->hora_salida_M);$i=strtotime('+30 minute',$i))
                @php                    
                    ...
                    $etados=$buscar_hora!=""?$estado[$buscar_hora]:"vacio";
                    $clase=$buscar_hora>0?'reservado':'libre';                    
                    $clase=$buscar_hora>0 && $etados=='atendido'?"ocupado":$clase; 
                    $boton=$buscar_hora>0 && $etados!='atendido'?'<button data-id="'.$buscar_hora.'" class="atendidos btn btn-secondary btn-sm float-right">Atendido</button>':'<button class="reservar btn btn-secondary btn-sm float-right">Reservar</button>'; 
                    $boton=$buscar_hora>0 && $etados=='atendido'?" ":$boton; 
                    
                @endphp                                
                <p class="horas-g {{$clase}}">{{$h}} <span class="nombre-pac">{{$apellido}} {{$nombre}}</span> <span>@php echo $boton @endphp</span></p>
        ...
        <div class="col-sm-6">
            ...
            @for($i=strtotime($horario->hora_entrada_T);$i<=strtotime($horario->hora_salida_T);$i=strtotime('+30 minute',$i))
                @php                    
                    $h=date('H:i:s',$i);
                    $time=$fecha.' '.$h;
                    $date=date('Y-m-d H:i:s',strtotime($time));
